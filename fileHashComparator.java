...
class fileHashComparator extends JFrame {
  ...
  private void btnCheckClick(ActionEvent evt){
    ...
    try {
        ...
        String hash = new String(Files.readAllBytes(Paths.get(fileC)));
        System.out.println(fileB +" - "+ getMD5Checksum(fileB));
        System.out.println("hash - " + hash);
        if (true) {
            if (getMD5Checksum(txtB.getText()).equals(hash)){
  ...
  private void btnBClick(ActionEvent evt){
    //Integer x,y,z;
    try{
      int returnVal = fc.showOpenDialog(fileHashComparator.this);
  ...
  public static void main(String[] args){
    fileHashComparator f = new fileHashComparator();
    f.setVisible(true);
  }
}